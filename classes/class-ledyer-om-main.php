...
class Ledyer_Order_Management_For_WooCommerce
{
	...
	public function actions()
	{
		...
		add_action("wc_ledyer_payments_supports", [$this, "add_gateway_support"]);
		add_action("lco_wc_supports", [$this, "add_gateway_support"]);

		// Capture an order -> lom-capture.php
		add_action("woocommerce_order_status_completed", function ($order_id, $action = false) {
		...
		add_action("woocommerce_order_status_cancelled", function ($order_id, $action = false) {
			lom_cancel_ledyer_order($order_id, $action, $this->api);
		});

		// Sync order items and totals
		add_action("woocommerce_saved_order_items", function ($order_id, $action = false) {
	...
	public function on_plugins_loaded()
	{
		...
		add_filter("plugin_action_links_" . plugin_basename(LOM_WC_MAIN_FILE), [$this, "plugin_action_links"]);
	}
