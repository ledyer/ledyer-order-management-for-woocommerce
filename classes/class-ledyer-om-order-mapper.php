...
class OrderMapper
{
	...
	private function get_item_reference($order_line_item)
	{
		if ("line_item" === $order_line_item->get_type()) {
			$product = $order_line_item["variation_id"]
				? wc_get_product($order_line_item["variation_id"])
				: wc_get_product($order_line_item["product_id"]);
			if ($product) {
				if ($product->get_sku()) {
	...
	{
		if (in_array($order_line_item->get_type(), ["line_item", "fee", "shipping"], true)) {
