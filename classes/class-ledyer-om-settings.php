...
class Ledyer_Order_Management_For_WooCommerce_Settings
{
	...
	public function __construct()
	{
		...
		// ledyer add-on.
		add_filter("ledyer_addons_settings_pages", [$this, "register_settings_page"], 1);
		add_action("ledyer_addons_settings_tab", [$this, "redirect_to_settings_page"], 999999);
	}
	...
	public function add_menu()
	{
		$submenu = add_submenu_page(
			"checkout-addons",
			__("Ledyer Order Management", "ledyer-order-management-for-woocommerce"),
			__("Ledyer Order Management", "ledyer-order-management-for-woocommerce"),
			"manage_woocommerce",
			"lom-settings",
			[$this, "settings_page"]
	...
	public function register_settings()
	{
		...
		add_settings_section(
			...
			[$this, "lom_settings_section_callback"],
			"lom-settings"
		);
		...
		add_settings_field(
			"lom_auto_capture",
			"On order completion",
			[$this, "field_auto_capture_render"],
			"lom-settings",
			"lom_settings_section"
		);
		add_settings_field(
		...
		add_settings_field(
			...
			"On fail set status",
			[$this, "field_status_error_ledyer_woo_render"],
			"lom-settings",
			"lom_settings_section"
		);
	}
	...
	public function field_auto_capture_render()
	{
		...
		<label for="lom_settings[lom_auto_capture]" >
			...
			<?php esc_html_e(
   	"Capture the Ledyer order automatically when WooCommerce order is marked complete.",
	...
	public function field_auto_cancel_render()
	{
		...
		<?php esc_html_e(
  	"Cancel the Ledyer order automatically when WooCommerce order is marked canceled.",
  	"ledyer-order-management-for-woocommerce"
  ); ?>
		</label>
		<?php
	...
	public function field_auto_update_render()
	{
		...
		<?php esc_html_e(
  	"Update Ledyer order automatically when WooCommerce order is updated.",
  	"ledyer-order-management-for-woocommerce"
  ); ?>
		</label>
		<?php
	...
	{
		$options = get_option("lom_settings");
		$current_value = isset($options["lom_status_mapping_ledyer_error"])
			? $options["lom_status_mapping_ledyer_error"]
			: "wc-on-hold";
  ...
  endforeach; ?>
	...
	public function add_page_tabs($current = "settings")
	{
   	...
   	$class = $tab === $current ? "nav-tab-active" : ""; ?>
					<a class="nav-tab <?php echo esc_html($class); ?>" href="?page=<?php echo esc_html(
	$pages[$tab]
	...
	{
		global $wp;
		$pages = apply_filters("ledyer_addons_settings_pages", []);
		$i = count($pages);
		?>
		<p>
