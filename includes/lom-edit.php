 ...
 */
	function edit_ledyer_order($order_id, $action = false, $api) {
		...
		$mapper = new \LedyerOm\OrderMapper($order);
		$data = $mapper->woo_to_ledyer_order_lines();
		...
		$errmsg = 'Ledyer order could not be updated due to an error: ' . $response->get_error_message();
		$order->add_order_note( $errmsg );
	}