...
function lom_refund_ledyer_order($order_id, $amount, $api)
{
	...
	return is_wp_error($response)
		? _lom_handle_wp_error($order, $response, "refunded")
		: _lom_process_successful_refund($order, $amount);
}
...
function _lom_can_order_be_refunded($order, $api)
{
	...
	if (
		is_wp_error($ledyer_payment_status_response) ||
		$ledyer_payment_status_response["status"] !== LedyerOmPaymentStatus::orderCaptured
	) {
